 ...
 * @returns {Element}
 */
function render (name, scope, node, partials) {
  var tmpl = getTemplate(name)

  ps = partials || render.defaultPartials
  if (!tmpl) return undefined
  el = node || cloneNodeAsElement(tmpl, tmpl.getAttribute('data-tagname') || 'div')

  if (window.Mustache && window.Mustache.render) {
    el.innerHTML = Mustache.render(tmpl.textContent, scope, ps)
  }
